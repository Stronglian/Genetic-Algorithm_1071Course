            ...
            for i in range(len(weightArr)): #輪盤每一格的判斷
                if ranTmp < sum(weightArr[:i+1]):
                    ...
                    if pairNumCount == self.tournamentSize:
                        ...
                        pairNumCount = 0
                    break
#        indexList = np.arange(len(inputArr))
#        np.random.shuffle(indexList)
#        print(np.random.rand(len(inputArr)) < weightArr/weightArr.sum())
        return pairGroup
...
#        print('Crossover','out-', newLi)
    ...
    def Mutation(self, inputStr):
        ...
        return newStr
#        #更久
#        FuncChange = lambda x : '0' if x == '1' else '1'
#        tmpStrArr = np.array(list(inputStr))
#        newStr = ''
#        randList = np.random.rand(len(inputStr)) < self.mutationRate
#        tmpStrArr = [ FuncChange(tmp) if randList[i] else tmp for i, tmp in enumerate(tmpStrArr) ]
#        newStr = newStr.join(tmpStrArr)
#        return newStr
    def MainFlow(self):
        """"""
        #儲存空間，String、Fitness
...
if __name__ == '__main__' :
    ...
    ans = test.MainFlow()
    print('Final::', 'String:',ans[0], '\nValue(x):', round(ans[1],5), '\nFitness:', round(ans[2], 5))
